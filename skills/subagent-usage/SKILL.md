...
name: subagent-usage
description: |
  Full usage rules for the pi-agents-tmux subagent tools (subagent,
  delegate_subagent, steer_subagent, get_subagent_result,
  wait_for_subagent_idle, stop_subagent, complete_subagent). Load when you
  need the complete calling rules, edge-case semantics (bg vs pane lanes,
  session reuse, timeouts, context-overflow retries, pane-cwd-stale), and
  best practices for orchestrating sub-agents.
---

# pi-agents-tmux — subagent usage
