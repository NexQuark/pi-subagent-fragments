...
Implementation: `extensions/subagent/agent-end-watchdog.ts` exposes `createAgentEndWatchdog(deps)`, and `settled-handler.ts` registers the Pi 0.80.4+ `agent_settled` lifecycle event. On that event, the child handles inbox-delivered tasks directly and scans the task registry for active (`queued`/`running`/`unknown`) bridge-delivered records belonging to its agent. After `VSTACK_AGENT_END_WATCHDOG_GRACE_SEC` (default 10s) the registry watchdog confirms the outbox is still missing, the task record is still active, and the pane is idle before writing a synthetic outbox via `O_EXCL` open at `completionPath(runtimeRoot, agent, taskId)` with `status: "needs_completion"`, `reason: "turn-ended-without-complete-subagent"`, and `synthetic: true`. Successful synthesis also calls `markTaskNeedsCompletion`, so the parent's existing wake/poll path picks the outbox up unchanged. The file name and env names remain for compatibility.
...
2. **Only sub-meta commits to `main`**, and only from the fixed `…-meta`
   worktree (or a temp worktree), never by checking out main in the
